...
# #get the video
# video_stream = cv2.VideoCapture('code/images/modi.mp4')
...
#     sample_faces = [detect_extract_face(plt.imread('code/dataset/training/modi/1.jpg')),
...
#         vggface_model = VGG16(include_top=False, model='resnet50', input_shape=(224,224,3), pooling='avg')   
...
def detect_extract_face(image_to_detect):
    ...
    for index, current_face_location in enumerate(all_face_locations):
        # splitting the tuple to get the four position values of the current face
        x, y, width, height = current_face_location['box']
        # start coordinates
        left_x, left_y = x, y
        # end coordinates
        ...
        current_face_image = current_face_image.resize((128, 128))
        # converting to numpy array
        current_face_image_np_array = np.asarray(current_face_image)
        # return array
        return current_face_image_np_array
...
# create VGGFace model outside the loop to avoid redundant initialization
vggface_model = VGG16(include_top=False, input_shape=(128, 128, 3), pooling='avg')
...
while True:
    ...
    if frame_count % frame_skip == 0:
        ...
        if sample_faces[1] is not None:
            ...
            # the face that needs to be verified
            image_to_be_verified = sample_faces_embeddings[0]
            # the faces that will be verified against
            image_to_verify = sample_faces_embeddings[1]
