...
# 1. load image
original_image = cv2.imread("C:/Users/ASUS/OneDrive/Desktop/Face Recognition Project/code/images/samples/Tom_Holland.jpg")
...
image_to_recognize = face_recognition.load_image_file("C:/Users/ASUS/OneDrive/Desktop/Face Recognition Project/code/images/samples/Tom_Holland.jpg")
...
def register_new_face(image_path,label,known_face_encodings,known_face_names):
    new_face_encodings = face_recognition.face_encodings(image_path)[0]
    known_face_encodings.append(new_face_encodings)
...
for curr_face_location, curr_face_encoding in zip(all_face_locations , all_face_encodings):
    ...
    if True in all_matches:
       ...
       name_of_person = known_face_names[first_match_index]
        ...
        cv2.putText(original_image , name_of_person , (left_pos , bottom_pos + 20) , font , 0.5 , (255 , 255 , 0) , 1)
        ...
        name_of_person = input("Enter the new person name : ")
        register_new_face(image_to_recognize,name_of_person,known_face_encodings,known_face_names)
        # 10. Compare faces & get the matches list(inside the loop)
        # See if the face is any mattch(es) for the known face(s)
        # by using ".compre_faces()" we can compare faces ".compare_faces("known_face_encodings","face_encoding_to_check")"
        all_matches = face_recognition.compare_faces(known_face_encodings, curr_face_encoding)
